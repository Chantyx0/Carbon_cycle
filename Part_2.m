...
% Read in data
Anthro_CO2 = xlsread('Anthro_CO2_Input.xlsx');
...
if (t>=2050)
    ...
    FF = interp1(Anthro_CO2(:,1),Anthro_CO2(:,3), t(n));
end


%Land use change
LU = interp1(Anthro_CO2(:,1),Anthro_CO2(:,4), t(n));
Soil_disrup = 0.25*LU;
Biomass_burn = 0.75*LU;
    ...
    if n < nsteps
        Atmospheric_CO2(n+1) = Atmospheric_CO2(n) + ( F_Soil_Respiration + F_Terrestrial_Biota_Respiration + F_Volcanic_CO2 + FF + Soil_disrup + Biomass_burn - F_CO2_Exchange - F_Terrestrial_Photosynthesis ) * dt ;
        ...
        Terrestrial_Biota(n+1) = Terrestrial_Biota(n) + ( F_Terrestrial_Photosynthesis - F_Terrestrial_Biota_Respiration - F_Litter_Fall - Biomass_burn ) * dt ;

        Soil(n+1) = Soil(n) + ( F_Litter_Fall - F_Soil_Respiration - F_Runoff - Soil_disrup ) * dt ;
